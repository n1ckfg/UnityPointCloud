...
#include "UnityCG.cginc" 
	...
	void GS_Main(point GS_INPUT p[1], inout TriangleStream<FS_INPUT> triStream) {
		...
		float sizeMult = (abs(p[0].pos.x) + abs(p[0].pos.y) + abs(p[0].pos.z)) / 100;
		float halfS = 0.5f * _Size * sizeMult;
	...
	float4 FS_Main(FS_INPUT input) : COLOR{
		return _SpriteTex.Sample(sampler_SpriteTex, input.tex0) * tex2D(_MainTex, input.tex1 * float2(1, 0.5) + float2(0, 0.5)) * _Color;
	}
